...
module.exports = (function(){
	...
	return function init( _bot ){
        ...
        bot.say = function(to, text, dontLog){
            ...
            if (!dontLog && to.indexOf('#') === 0) {
            //if (!dontLog) {
                bot.countMessage(to, bot.botName, text);
		...
		bot.on('nick', function(oldNick, newNick) {
			if (oldNick === bot.botName) {
				bot.botName = newNick;
			}
		});

		bot.on('registered', function (message) {
			bot.botName = message.args[0];
		});
		...
		bot.on('message', function( from, channel, text, message){
            ...
            } else if (text.indexOf('#changenick') === 0) {
                if (parts.length === 3 && parts[1] === bot.botName) {
                    bot.ops.isOp(message.user, function (err, data) {
                        if (data !== 0) {
                            bot.send('NICK', parts[2]);
                        }
                    });
                }
            } else if (text.indexOf('#restart') === 0 || text.indexOf('#kill') === 0) {
                bot.ops.isOp(message.user, function (err, data) {
                    if (data !== 0) {
                        if (parts.length === 2 && parts[1] === bot.botName) {
	                        bot.say(channel, 'restarting...');
	                        setTimeout(function () {
		                        process.exit(1);
	                        }, 2000);
                        } else {
	                        bot.say(channel, 'are you talking to me? use ' + parts[0] + ' ' + bot.botName);
                        }
                    } else {
	                    bot.say(channel, 'http://i.imgur.com/4C7iu09.gif');
                    }
                });
            }
		});
    ...
    function setCooldown (key, timeoutInSeconds) {
        ...
        _.each(key, function(item) {
            redis.hset(bot.botID + '.cooldown', item, Date.now() + (timeoutInSeconds * 1000));
        });
    }
    ...
    function isOnCooldownHelper (key, callback) {
        redis.hget(bot.botID + '.cooldown', key, function(err, data) {
            if (err) {
    ...
    function clearAllCooldowns () {
        redis.del(bot.botID + '.cooldown', function(err, data) {
            if (err) {
